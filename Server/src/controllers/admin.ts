...
export const getAllPendingSellers = asyncHandler(async (req, res) => {
  const result = await db
    .select()
    .from(seller)
    .where(eq(seller.isVerified, false));
...
export const getSellerById = asyncHandler(async (req, res) => {
  ...
  const result = await db
    .select()
    .from(seller)
    .where(eq(seller.id, id as string));
  if (!result) {
...
export const makeSellerVerified = asyncHandler(async (req, res) => {
  ...
  await dbPool.transaction(async (trx) => {
    const result = await trx
      .update(seller)
      .set({ isVerified: true })
      .where(and(eq(seller.id, id), eq(seller.isVerified, false)))
      .returning();
    if (!result[0]) {
...
export const blockSeller = asyncHandler(async (req, res) => {
  ...
  const result = await db
    .update(seller)
    .set({ isActive: false })
    .where(and(eq(seller.id, id), eq(seller.isActive, true)))
    .returning();
  if (!result[0]) {
...
export const unBlockSeller = asyncHandler(async (req, res) => {
  ...
  const result = await db
    .update(seller)
    .set({ isActive: true })
    .where(and(eq(seller.id, id), eq(seller.isActive, false)))
    .returning();
  if (!result[0]) {
