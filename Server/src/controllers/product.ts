...
export const getAllProductsOfSeller = asyncHandler(async (req, res) => {
    ...
    const result = await db.select().from(product).where(eq(product.userId, user.id))
...
export const getSellerProductById = asyncHandler(async (req, res) => {
    ...
    const result = await db.select().from(product).where(and(eq(product.id, id), eq(product.userId, user.id)))
    if (!result[0]) {
