...
export const trackActivity = asyncHandler(async (req, res) => {
  ...
  const { productId, actionType } = req.body as {
    productId: string;
...
export const getRecommendations = asyncHandler(async (req, res) => {
  ...
  const recommendationsKey = `recommendation:${user.id}`;
  const cached = await redis.get(recommendationsKey);
  if (cached) {
    return res.json(
      new ApiResponse(
        200,
  ...
  if (viewedProductIds.length === 0) {
    const fallbackProducts = await db
      ...
      .limit(5);
    return res.json(
      new ApiResponse(200, fallbackProducts, "Fallback recommendations"),
    );
  }
  ...
  const viewedProducts = await db
    ...
    .from(product)
    .where(inArray(product.id, viewedProductIds));

  const userCategories = [
    ...new Set(viewedProducts.flatMap((p) => p.category || [])),
  ];
  ...
  const candidateProducts = await db
    ...
    .where(
      and(
        arrayOverlaps(product.category, userCategories),
        eq(product.isActive, true),
        notInArray(product.id, viewedProductIds),
      ),
    )
    .limit(30);
  ...
  if (candidateProducts.length === 0) {
    return res.json(
      new ApiResponse(200, [], "No new recommendations available"),
    );
  }
  ...
  const recommendations = candidateProducts.filter((p) =>
